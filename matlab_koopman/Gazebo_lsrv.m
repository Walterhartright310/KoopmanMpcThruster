...
% %% ************************** Basis functions *****************************
% 
n=1;% the number of states
m=1;%the input number
basisFunction = 'rbf';
% RBF centers
Nrbf = 3;
cent = rand(n,Nrbf)*1 -0.5;
rbf_type = 'thinplate'; 
...
data = readtable('odom_data_thruster.csv');
% Identify rows with empty or NaN values in any column
%columnsToCheck = data(:, 3:4);  % Extract columns 3 and 4
%dataAsNumeric = table2array(data(:, 3:4));
%isEmptyOrNaN = any(isnan(dataAsNumeric), 1);
...
% 将三个矩阵放在一起保存到一个字典中
data = struct('Alift', Alift, 'Blift', Blift, 'Clift', Clift,'Nrbf',Nrbf,'rbf_type',rbf_type,'n',n,'m',m,'cent',cent);
save('mpc_matrix.mat', '-struct', 'data');
...
X_plot=[];
 for i = 0:Nsim-1
%     % Koopman predictor
     xlift = [xlift, Alift*xlift(:,end) + Blift*U(i+1)]; % Lifted dynamics
     U_plot=[U_plot,U(i+1)];
     X_plot=[X_plot,X(i+1)];
%     % True dynamics
...
% 
 figure
 plot([0:Nsim-1]*deltaT,U_plot,'linewidth',lw); hold on
% title('Control input $u$', 'interpreter','latex'); xlabel('Time [s]','interpreter','latex');
% set(gca,'fontsize',20)
% 
 figure
 plot([0:Nsim]*deltaT,X(1:31),'linewidth',lw); hold on
